...
parser.add_argument('--load-partmodel', default='', type=str)
...
def main():
    ...
    elif args.load_partmodel:
        load_part_model(args.load_partmodel, model)
    ...
    tracking_module = TrackingModule(model, optimizer, criterion,
                                     config.det_type)
    # if args.evaluate:
    #     logger.info('Evaluation on training set:')
    ...
    #     return
    train(train_loader, val_dataset,  tracking_module,
          lr_scheduler, last_iter + 1, tb_logger)


def train(train_loader, val_loader,  tracking_module : TrackingModule,
    ...
    for i, (input, det_info, det_id, det_cls,
        ...
        tracking_module.model.encoder.drop_path_prob = 0.2 * i / len(train_loader)
