...
def load_partial_state(pc_path, im_path, model, optimizer=None, rank=0):

    def map_func(storage, location):
        return storage.cuda()

    if os.path.isfile(pc_path) and os.path.isfile(im_path):
        if rank == 0:
            print("=> loading checkpoint for pointnet++ '{}' and vgg '{}'".format(pc_path,im_path))

        pc_model = torch.load(pc_path, map_location=map_func)
        im_model = torch.load(im_path, map_location=map_func)
        model_dict = model.state_dict() 
        model_dict.update(pc_model)
        model.load_state_dict(model_dict, strict=False)

    else:
        if rank == 0:
            print("=> no checkpoint found at '{}' and '{}'".format(pc_path, im_path))

def load_part_model(path, model, rank=0):
    def map_func(storage, location):
        return storage.cuda()

    if os.path.isfile(path):
        if rank == 0:
            print("=> loading part checkpoint from '{}'".format(path))

        part_model = torch.load(path, map_location=map_func)
        model_dict = model.state_dict() 
        model_dict.update(part_model)
        model.load_state_dict(model_dict, strict=False)
