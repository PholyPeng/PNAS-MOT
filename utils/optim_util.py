...
def split_bn_bias(layer_groups):
    "Split the layers in `layer_groups` into batchnorm (`\`) and non-batchnorm groups."
    split_groups = []
...
def model_g2master_g(model_params,
                     master_params,
...
class OptimWrapper():
    ...
    def create(cls, opt_func, lr, layer_groups, **kwargs):
        ...
        opt = opt_func([{
            ...
            'lr': lr
        } for l in split_groups])  # modified for adabound
        #opt = opt_func([{'params': trainable_params(l), 'lr': lr} for l in split_groups])
        opt = cls(opt, **kwargs)
